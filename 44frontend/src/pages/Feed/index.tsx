            ...
            <section className={styles.feed_info}>
                {
                    feed?.map((item, index: number, array: feedSchema[]) => {
                        return (
                            <Panel key={index} icon={urlFor(item.logo.asset._ref).url()} text={item.publicationName} isExternalLink={true} subtitle={item.title} noInfo={true} action="link" url={item.link} />
                        )
                    })
                }
            </section>
